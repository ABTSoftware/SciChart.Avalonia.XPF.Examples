...
namespace SciChart.Examples.Demo.Helpers.ProjectExport
{
    public static class ProjectWriter
    {
        ...
        private static string WriteProjectFiles(Example example, string exportPath, string libsPath, bool isXpfExport, string xpfLicenseKey)
        {
            ...
            files[MainWindowFileName] = GenerateWindowFile(files[MainWindowFileName], example).Replace("[EXAMPLE_TITLE]", exampleTitle);
            ...
            try
            {
                ...
                {
                    using (var sw = new StreamWriter(Path.Combine(exportDirPath, file.Key)))
                    {
                        sw.Write(file.Value);
                    }
                }
        ...
        */
        ...
        private static string GetFileNameFromNs(string fullName)
        {
            return fullName
                ...
                .Replace("SciChart.Examples.Demo.Xpf.Helpers.ProjectExport.Templates.", string.Empty)
                .Replace(".c.", ".cs.")
